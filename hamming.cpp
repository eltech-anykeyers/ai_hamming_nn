...
const double Hamming::epsilon = 0.1;
...
void Hamming::append(const MarkedDrawer& image)
{
...
QString Hamming::recognize(const MarkedDrawer& image)
{
    QVector<double>* data = new QVector<double>();
    for (const auto& pixel : image.getPixelData())
    {
...
qint32 Hamming::getImageLinearSize() const
{
    return this->imageLinearSize;
}

void Hamming::setLinearSize(const qint32 imageLinearSize)
{
    this->imageLinearSize = imageLinearSize;
    this->randomShittyParameter = this->imageLinearSize / 2.0;
}

QSize Hamming::getImageSize() const
{
    return this->imageSize;
}

void Hamming::setImageSize(const QSize& imageSize)
{
    this->imageSize = imageSize;
    this->setLinearSize(this->imageSize.width() * this->imageSize.height());
}

void Hamming::updateWeightsMatrix()
{
    weightsMatrix->clear();
...
void Hamming::updateFeedbackMatrix()
{
...
double Hamming::activation(double arg) const
{
    if (arg <= 0.0)
        return 0.0;
...
double Hamming::norm(const QVector<double>& vector) const
{
